  ...
  return {
   ...
   pendingSceneTargets,
    mediaExecution: {
      ...
      scenes: { kind: "platform-media-generation", generate: "recut.video.generate_async", complete: "recut.media.get_job -> assetId -> resource.create", alternatives: "Use a composition extension only when the user explicitly requests that composition; do not substitute it for generated video." },
    },
   inFlight: null,
  };
}
...
function prepareResource(input, ctx) {
  ...
  const stageWorkflow = {
    ...
    scenes: `视频生成昂贵，默认只生成第一段并停下等待用户确认。${batch ? "用户已明确要求一次生成全部剩余段；逐段生成，但每段必须独立调用 resource.create，绝不能合并为一个含多段的 Scene resource。" : "不要循环生成其余段，不要合并为一个含多段的 Scene resource。"} 执行顺序不可替换：先调用 recut.video.generate_async，传入该段 keyframe.assetId 与 audio.assetId；再轮询 recut.media.get_job 至 completed；最后调用 resource.create 保存独立 Scene resource。禁止使用 HyperFrames、ffmpeg、浏览器自动化、终端脚本或本地渲染替代该 API 调用。顶层字段为 beatId、title、durationSec、visualAction、cutPoint、video；video 必须是完整 MediaSnapshot 且 assetId 指向刚生成的视频。`,
 ...
 const interfaceText = contract ? `输入：${contract.inputs.join("；")}\n输出对象：${outputFields}${itemFields}` : "输出：一份可审阅的阶段资源。";
  const executionMethod = kind.toLowerCase() === "scenes" ? "这是平台视频生成任务：先 recut.video.generate_async，再 recut.media.get_job，最后 resource.create。除非用户明确要求 HyperFrames，否则不要把它解释为 HyperFrames 合成任务。" : kind.toLowerCase() === "keyframes" ? "这是平台图片生成任务：调用 recut.image.generate。" : kind.toLowerCase() === "audio" ? "这是平台语音生成任务：调用 recut.speech.generate_async 后轮询。" : "遵循当前阶段契约。";
 return {
