...
function prepareResource(input, ctx) {
  ...
  const stageWorkflow = {
    ...
    keyframes: "每个节拍一张关键画面；一个关键画面只服务约 5 秒的单一信息变化，绝不以一张关键画面覆盖 10 秒或更长的叙事。先读取 recut.project_context，按当前图片生成方案逐张生成。平台图片路线使用 recut.image.generate 提交异步 job，立即取得稳定 jobId 与排队中 assetId，并用 recut.media.wait_for_job 等它 completed 后再保存；Codex 原生方案使用宿主提供的图片能力，随后将最终文件写入当前 Recut 项目目录并调用 recut.media.import_image。无论哪种路径，图片必须先成为稳定的 Recut 媒体素材并取得真实 assetId，再保存；绝不伪造 assetId 或只交付对话预览，也绝不把排队中/生成中的素材当作已完成。每个 keyframes[] 项的 image 必须是完整 MediaSnapshot，image.assetId 指向该节拍已完成的新图；text 是该图的原始提示词；imageAssetIds 可引用真实视觉参考素材；audioAssetIds 为空数组。不要保存其他 section 的资源 ID；所需上下文来自 workflow.context。绝不把文字画面描述当成关键画面保存。",
    ...
    scenes: `视频生成昂贵，默认只生成第一段并停下等待用户确认。每个场景视频只对应一个约 5 秒的场景与关键画面；时长更长时继续拆分，不得靠一个场景视频覆盖多个信息变化。${batch ? "用户已明确要求一次生成全部剩余段；逐段生成，但每段必须独立调用 resource.create，绝不能合并为一个含多段的场景视频资源。" : "不要循环生成其余段，不要合并为一个含多段的场景视频资源。"} 执行顺序不可替换：先调用 recut.video.generate，传入该段 keyframe.assetId 与已完成 audio.assetId。调用的视频提示词必须把该场景 audio.text（若为空则 narration）原样逐字写在“唯一人声/逐字台词”段落中，并明确：使用附带参考音频作为唯一人声；禁止新增对白、歌词、耳语、翻译或不可辨识人声；只生成与画面动作相符的非语言效果声。若路线使用 Seedance，output.generateAudio 默认为 true，只有用户明确要求无声视频才传 false；Gemini 不传此字段。提交响应会带稳定 assetIds[0]，立刻调用 resource.create 保存独立场景视频资源，不能等待轮询完成。recut.media.get_job 只用于读取并报告该素材的排队中/生成中/已完成/失败状态。禁止使用 HyperFrames、ffmpeg、浏览器自动化、终端脚本或本地渲染替代该 API 调用。顶层字段为 beatId、title、durationSec、visualAction、cutPoint、video；video 必须是完整 MediaSnapshot 且 assetId 指向刚提交的视频。`,
 ...
 const interfaceText = contract ? `输入：${contract.inputs.join("；")}\n输出对象：${outputFields}${itemFields}` : "输出：一份可审阅的阶段资源。";
  const executionMethod = kind === "scenes" ? "这是平台视频生成任务：recut.video.generate 成功提交后立刻用返回的排队中 assetIds[0] 调用 resource.create；常驻服务原位更新状态。除非用户明确要求 HyperFrames，否则不要把它解释为 HyperFrames 合成任务。" : kind === "keyframes" ? "这是按平台配置执行的异步图片生成任务：先读取 recut.project_context；平台图片路线调用 recut.image.generate 提交 job，立即取得稳定 jobId 与排队中 assetId，并用 recut.media.wait_for_job 等它 completed 后再保存；Codex 原生方案使用宿主提供的图片能力，然后将最终文件写入当前项目并调用 recut.media.import_image。无论哪种路径，先确保图片成为已完成的稳定 Recut 媒体素材并取得真实 assetId，再保存；绝不只交付对话预览、伪造 ID 或把生成中的素材当已完成。" : kind === "audio" ? "这是平台语音生成任务：调用 recut.speech.generate 后立刻保存返回的稳定 assetId；常驻服务原位更新状态。" : "遵循当前阶段契约。";
 return {
