  ...
  return {
   ...
   pendingSceneTargets,
    ...
    mediaExecution: {
      ...
      keyframes: { kind: "平台图片生成", generate: "读取 recut.project_context；按配置使用 recut.image.generate 或宿主的 Codex 原生图片能力；所有原生结果必须经 recut.media.import 归档", complete: "在 resource.create 之前，原生结果须先写入当前项目并获得稳定图片 assetId" },
      audio: { kind: "平台媒体生成", generate: "recut.speech.generate", complete: "提交成功 → 排队中 assetIds[0] → resource.create；常驻服务原位更新素材状态" },
      scenes: { kind: "平台媒体生成", generate: "recut.video.generate", complete: "提交成功 → 排队中 assetIds[0] → resource.create；常驻服务原位更新素材状态；Seedance 默认 output.generateAudio=true，除非用户明确要求无声视频；视频文本必须逐字引用 audio.text，禁止新增人声", alternatives: "仅在用户明确要求代码化合成时才使用合成扩展，不得以其替代生成视频。" },
    },
   inFlight: null,
  };
}
...
function prepareResource(input, ctx) {
  ...
  const stageWorkflow = {
    research: "先建立足够的证据库，再讨论故事。检索文章、YouTube、小红书、抖音、论文或原始资料；每条来源必须先用 recut.media.import 登记为全局 reference 素材：URL 只是身份，还要尽可能提交真实内容与完整元数据——文章/网页附 content 全文（真正可审阅的文章数据），有代表性的图片附 imageData(base64)（真正的图片数据），YouTube 等视频平台至少补齐频道名、频道 URL、时长、播放/点赞与描述；并保存规范的来源类型、标题、作者和发布时间。资料研究仅保存 assetId、标题、类型、与本片相关的事实洞见和可信度/偏见判断。至少收集 3 条不同来源、覆盖支持与反例/限制后保存 status: 'draft' 并停下，绝不提前写剧本。",
    ...
    keyframes: "每个节拍一张关键画面；一个关键画面只服务约 5 秒的单一信息变化，绝不以一张关键画面覆盖 10 秒或更长的叙事。先读取 recut.project_context，按当前图片生成方案逐张生成。平台图片路线使用 recut.image.generate 提交异步 job，立即取得稳定 jobId 与排队中 assetId，**先落位、不空等**：拿到 assetId 立刻用 resource.create 保存本阶段资源（常驻服务原位更新状态、UI 自动切换），不要用 recut.job.wait 把保存堵在终态之后；Codex 原生方案使用宿主提供的图片能力，随后将最终文件写入当前 Recut 项目目录并调用 recut.media.import 取得真实 assetId。绝不伪造 assetId 或只交付对话预览；只有下一步真的依赖图片内容（读图决策、连续性/身份或质量验收、用于导出）时才 wait。每个 keyframes[] 项的 image 必须是完整 MediaSnapshot，image.assetId 指向该节拍已完成的新图；text 是该图的原始提示词；imageAssetIds 可引用真实视觉参考素材；audioAssetIds 为空数组。不要保存其他 section 的资源 ID；所需上下文来自 workflow.context。绝不把文字画面描述当成关键画面保存。",
    audio: "每个场景都必须先生成真实可播放的旁白：从 recut.project_context.media.defaultRoutes 找到 speech.generate 的 credentialId，调用 recut.media.list_voices 取得 voiceId；逐段调用 recut.speech.generate 后立即取得稳定 assetId 并保存 scenes[]。每项 audio 必须是完整 MediaSnapshot：assetId 为刚提交的音频，text 为旁白原文，imageAssetIds/audioAssetIds 为空数组。不要保存其他 section 的资源 ID；所需上下文来自 workflow.context。常驻服务会原位更新状态；只有音频已完成时才可将它作为场景视频输入。音乐和音效可以是编辑指令，但不能替代旁白音频。生成失败时不得保存空声音设计资源，应如实报告。",
    scenes: `视频生成昂贵，默认只生成第一段并停下等待用户确认。每个场景视频只对应一个约 5 秒的场景与关键画面；时长更长时继续拆分，不得靠一个场景视频覆盖多个信息变化。${batch ? "用户已明确要求一次生成全部剩余段；逐段生成，但每段必须独立调用 resource.create，绝不能合并为一个含多段的场景视频资源。" : "不要循环生成其余段，不要合并为一个含多段的场景视频资源。"} 执行顺序不可替换：先调用 recut.video.generate（必须显式传 mode:"generate"——本 App 自带“默认只生成第一段并等用户确认”的门禁，跳过平台的通用提案 gate），传入该段 keyframe.assetId 与已完成 audio.assetId。调用的视频提示词必须把该场景 audio.text（若为空则 narration）原样逐字写在“唯一人声/逐字台词”段落中，并明确：使用附带参考音频作为唯一人声；禁止新增对白、歌词、耳语、翻译或不可辨识人声；只生成与画面动作相符的非语言效果声。若路线使用 Seedance，output.generateAudio 默认为 true，只有用户明确要求无声视频才传 false；Gemini 不传此字段。提交响应会带稳定 assetIds[0]，立刻调用 resource.create 保存独立场景视频资源，不能等待轮询完成。recut.job.status 只用于读取并报告该素材的排队中/生成中/已完成/失败状态。禁止使用 HyperFrames、ffmpeg、浏览器自动化、终端脚本或本地渲染替代该 API 调用。顶层字段为 beatId、title、durationSec、visualAction、cutPoint、video；video 必须是完整 MediaSnapshot 且 assetId 指向刚提交的视频。`,
 ...
 const interfaceText = contract ? `输入：${contract.inputs.join("；")}\n输出对象：${outputFields}${itemFields}` : "输出：一份可审阅的阶段资源。";
  const executionMethod = kind === "scenes" ? "这是平台视频生成任务：recut.video.generate 显式传 mode:'generate' 直生（本 App 自带确认门禁，不走通用提案），成功提交后立刻用返回的排队中 assetIds[0] 调用 resource.create；常驻服务原位更新状态。除非用户明确要求 HyperFrames，否则不要把它解释为 HyperFrames 合成任务。" : kind === "keyframes" ? "这是按平台配置执行的异步图片生成任务：先读取 recut.project_context；平台图片路线调用 recut.image.generate 提交 job，立即取得稳定 jobId 与排队中 assetId，拿到 assetId 立刻调用 resource.create 保存本阶段资源（先落位、不空等，常驻服务原位更新状态），不要用 recut.job.wait 把保存堵在终态之后；Codex 原生方案使用宿主提供的图片能力，然后将最终文件写入当前项目并调用 recut.media.import 取得真实 assetId。绝不只交付对话预览或伪造 ID；只有下一步依赖图片内容时才 wait。" : kind === "audio" ? "这是平台语音生成任务：调用 recut.speech.generate 后立刻保存返回的稳定 assetId；常驻服务原位更新状态。" : "遵循当前阶段契约。";
 return {
