...
function workflowContext(_, ctx) {
  ...
  return {
   ...
   pendingSceneTargets,
    mediaExecution: {
      keyframes: { kind: "configured-image-generation", generate: "read recut.project_context; use recut.image.generate for a Media Platform route or the host's Codex-native image capability when configured; archive every native final through recut.media.import_image", complete: "stable Recut image assetId; a native result is written into the current project then imported before resource.create" },
...
function prepareResource(input, ctx) {
 ...
 const interfaceText = contract ? `输入：${contract.inputs.join("；")}\n输出对象：${outputFields}${itemFields}` : "输出：一份可审阅的阶段资源。";
  const executionMethod = kind.toLowerCase() === "scenes" ? "这是平台视频生成任务：recut.video.generate_async 成功提交后立刻用返回的 queued assetIds[0] 调用 resource.create；常驻 Daemon 原位更新状态。除非用户明确要求 HyperFrames，否则不要把它解释为 HyperFrames 合成任务。" : kind.toLowerCase() === "keyframes" ? "这是按平台配置执行的图片生成任务：先读取 recut.project_context；Media Platform route 调用 recut.image.generate，Codex 原生方案使用宿主提供的图片能力，然后将最终文件写入当前项目并调用 recut.media.import_image。无论哪种路径，先确保图片成为稳定的 Recut Media Asset 并取得真实 assetId，再保存；绝不只交付对话预览或伪造 ID。" : kind.toLowerCase() === "audio" ? "这是平台语音生成任务：调用 recut.speech.generate_async 后立刻保存返回的稳定 assetId；Daemon 原位更新状态。" : "遵循当前阶段契约。";
 return {
