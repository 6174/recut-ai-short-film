...
export function DialogContent({ children, className, ...props }: DialogPrimitive.DialogContentProps) { const locale = useRecutLocale(); return <DialogPrimitive.Portal><DialogPrimitive.Overlay className="fixed inset-0 z-[90] bg-black/30 backdrop-blur-[1px]" /><DialogPrimitive.Content className={join("fixed left-1/2 top-1/2 z-[100] grid max-h-[88vh] w-[calc(100%-2rem)] max-w-3xl -translate-x-1/2 -translate-y-1/2 gap-5 overflow-y-auto rounded-xl border bg-card p-5 shadow-2xl outline-none", className)} {...props}><DialogPrimitive.Close aria-label={t(locale, "ui.close")} className="absolute right-4 top-4 rounded-sm text-muted-foreground hover:text-foreground"><X className="size-4" /></DialogPrimitive.Close>{children}</DialogPrimitive.Content></DialogPrimitive.Portal>; }
...
export function EmptyState({ action, label }: { action: () => void; label: string }) { const locale = useRecutLocale(); return <div className="grid min-h-64 place-items-center rounded-xl border border-dashed bg-card p-8 text-center"><div><div className="mx-auto grid size-10 place-items-center rounded-full bg-muted"><FolderPlus className="size-5 text-muted-foreground" /></div><h3 className="mt-4 text-sm font-semibold">{t(locale, "ui.emptyTitle")}</h3><p className="mt-1 max-w-xs text-sm leading-6 text-muted-foreground">{t(locale, "ui.emptyBody")}</p><Button className="mt-4" onClick={action}><FolderPlus className="size-4" />{label}</Button></div></div>; }
