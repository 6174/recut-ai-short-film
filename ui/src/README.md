...
成员清单
main.tsx: B-roll 工作台编排层；以两列等宽 panel 网格同时展示七个阶段；首次进入无 Brief 项目时打开起始表单，保存选题方向、细节描述与预期时长后交给 Codex 按 5 秒节拍创建 Beats；资源事件后刷新并按资源 ID 同步已打开详情，避免 resource.update 后弹窗继续显示旧快照。
...
resource-dialogs.tsx: iframe 内受控资源详情模态框、受确认的删除入口与新建请求弹窗；Brief 弹窗固定收集选题方向、细节描述和可选/自定义时长，并明确约 5 秒关键画面节奏；Look 弹窗强调参考图必须覆盖全片视觉元素；引用资源展示实际图片、视频画面或文字摘要。
resource-view.tsx: 资源展示语义层；按阶段把内部 JSON 翻译为图文、缩略文本、视频画面、镜头卡与清单，Brief 同时展示细节描述与预期时长；优先读取独立 Scene 的 `video.assetId` 与 Delivery 的最终 `assetId`，兼容历史 `scenes[]/shots[]` 项内的 `video.assetId` 或 `videoAssetId`；所有真实媒体从共享 Asset SSE 缓存读取状态，任何生成中的图片、音频或视频都显示实时计时，终态仅展示后端 `generationDurationMs`，历史素材绝不根据时间戳猜测生成耗时，也不直接向用户展示 JSON。
