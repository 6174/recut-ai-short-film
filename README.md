...
服务边界
此 App 只覆盖 B-roll：主题到 Vox 风格纸质拼贴解说片。平台不理解 Brief 数据结构；`background.js` 通过 SQLite/Artifact capability 自行实现。Agent 的 `generate_brief` MCP 工具和其他 App 的 `brief.create` API 共用同一业务函数。
