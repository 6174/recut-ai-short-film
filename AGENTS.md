...
首个 Beat 必须在三秒内给出结果承诺、反常识、直接问题、痛点或警示，绝不从背景介绍开场。**5 秒是关键画面与场景的默认节拍单位：一个 Beat、一个 Keyframe、一个 Scene 只承载约 5 秒和一个信息变化。**30 秒拆为 6 段、60 秒拆为 12 段、90 秒拆为 18 段、120 秒拆为 24 段；如果内容更长，增加节拍，绝不让一张关键画面覆盖 10 秒或更长叙事。回报点可使用静态或近静态镜头，让运动突然收束以强调结论。
...
回答：选题方向是什么、有哪些细节补充、讲给谁、核心观点是什么、观众为什么现在要看、整支片的张力来自哪里、预期总时长是多少。

请求目标：把项目起始表单中的选题方向、细节描述和预期时长收敛为后续所有创作共享的单一论点，不写成泛泛的选题介绍。交付应说明目标观众、可被验证的核心观点、开场冲突、因果推进方向和预期时长。完成标准：后续 Beats 必须按约 5 秒一段直接拆分（30/60/90/120 秒分别对应 6/12/18/24 段），每一段都能回答“这段为什么值得看”。没有 Brief 时不能靠猜测先做画面。

**Beats｜叙事节拍**

把观点排成能看完的短片结构。先用三秒内的冲突或反常识抓人，再以约 5 秒一个短节拍推进因果；每个节拍都要说明观众看见什么、理解什么。

请求目标：将 Brief 变成可执行的叙事骨架，而不是一组并列的提纲。每个节拍要有稳定的 `id`、标题、旁白要点、画面证据、叙事目的和时长；每个 `durationSec` 目标为 5，所有节拍总长必须等于 Brief 的预期时长。前一段提出的问题必须由后续段落回应。完成标准：开头三秒有明确钩子，中段每一节只推进一个信息变化，结尾回收核心论点；未确认 Beats 不能进入 Look 或 Keyframes。

**Look｜视觉风格**

一张风格参考图定义整支片的视觉语言，不是装饰海报，更不是单纯色彩样本。它必须同时呈现整支视频会反复出现的全部视觉元素：主体人物或产品、核心道具、信息卡、背景材料、装饰碎片、标题区域、纸张层次、色彩、纹理与情绪；后续 Keyframe 能从这张图直接继承完整视觉词汇。给出 3 个明显不同、都贴合主题的候选；每个候选必须有自己的参考图和原始画面描述。

请求目标：为同一叙事提出三种可比较的整片视觉系统，每个候选都必须先生成一张 16:9 图片。每张图必须保存自己的 `media.assetId` 与原始提示词，并在同一画面中覆盖全片所需的主体、核心道具、信息卡、背景材料、装饰碎片、标题安全区、纸张材料、色板与纹理；不得只交付色彩风格、纹理板或情绪海报。候选之间要在色彩、材料或信息密度上真实不同，不能只换形容词。完成标准：用户选择一个 Look 后，Keyframes 可以把它的图片作为 `image.imageAssetIds` 风格输入；未选定 Look 不得生成 Keyframes。

**Keyframes｜关键画面**

把每个约 5 秒的节拍变成一张能独立成立的画面。关键画面只服务一个信息变化；更长的段落必须先拆 Beat，再生成多张图。Keyframes 的请求目标是“多张图 + 每张图的创作说明”，不是“多段镜头文字”。
...
最终导出是确定性的例外：用户在 Delivery 面板手动选择 Scene 视频与可选 Audio 声音，并在固定视频轨、固定音频轨中排列顺序；面板直接调用 `delivery.export → ctx.media.compose`，由 Recut 平台受控的 FFmpeg 合成为一个新的 video Asset。视频轨的原生声音必须按顺序保留；用户额外选择的音频轨与原声混合，绝不能覆盖或静音视频原声。不要把时间线、尺寸或编码设置交给 Agent 决策；AI 只可在导出报错时处理 FFmpeg 安装、编码或素材诊断。平台优先自动安装缺失的 FFmpeg，不能安装时必须给出可执行原因并等待用户重试。
